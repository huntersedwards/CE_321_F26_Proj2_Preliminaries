...
def FindSharedNode(bar_1,bar_2):
    if bar_1.init_node == bar_2.init_node:
       ...
       return bar_1.end_node
        ...
        sys.exit("These two bars do not share a node.")
...
def FindOtherNode(node,bar):
    if (bar.end_node == node):
        ...
        sys.exit("The node that you input is not on the input bar.")
    return
...
def BarsToVectors(bar_1,bar_2):
    Node = FindSharedNode(bar_1, bar_2)
    vec1 = BarNodeToVector(bar_1, Node)
    vec2 = BarNodeToVector(bar_2, Node)
    return vec1, vec2

# Cross product of two vectors
def TwoDCrossProduct(vec1,vec2):
    Cross_Product = vec1(0)*vec2(1)-vec1(1)*vec2(2)
...
def CosineVectors(local_x_vec,other_vec):
    ...
    Norm_vec_1 = VectorTwoNorm(local_x_vec)
    Norm_vec_2 = VectorTwoNorm(other_vec)
    Cosine = Dot_Product / (Norm_vec_1 * Norm_vec_2)
    return Cosine
...
def SineVectors(local_x_vec,other_vec):
    ...
    Norm_vec_1 = VectorTwoNorm(local_x_vec)
    Norm_vec_2 = VectorTwoNorm(other_vec)
    Sine = Cross_Product / (Norm_vec_1 * Norm_vec_2)
    return Sine
